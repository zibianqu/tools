...
<head>
<meta charset="utf-8">
<title>同步数据-<?php echo C('site_name');?></title>
			...
			<div class="row-fluid">
				<div class="well">
					<div id="myTabContent" class="tab-content">
						<div class="tab-pane active in" id="home">
							<form id="tabs" action="<?php echo U('/admin/index/synchroData');?>" method="post">
								<label>缓存类型：</label> 
								<input type="checkbox" value="0" name="redis" id="redis" onclick="if($(this).is(':checked')){$(this).val(1);}else{$(this).val(0);} "/>redis
								<input type="hidden" name="synchro" value="1" />
								<div class="btn-toolbar">
									<a href="#myModal" data-toggle="modal" class="btn">同步</a>
				...
				<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					<div class="modal-header">
						...
						<h3 id="myModalLabel">同步数据</h3>
					</div>
					<div class="modal-body">
						<p class="error-text">
							<i class="icon-warning-sign modal-icon"></i>确定同步数据？
						</p>
					</div>
					<div class="modal-footer">
						<button class="btn" data-dismiss="modal" aria-hidden="true">取消</button>
						<button class="btn btn-danger" data-dismiss="modal" onclick="if($('#redis').val()!=1){alert('请选择类型');}else{$('#tabs').submit();}">确定</button>
					</div>
				</div>
			</div>
		</div>
